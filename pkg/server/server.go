...
func NewAuthServer(pocketClient *pocket.Client, tokenRepository rep.TokenRepository, redirectURL string) *AuthServer {
...
func (s *AuthServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	requestToken, err := s.tokenRepository.Get(chatID, rep.RequestTokens)
	if err != nil {
	...
	err = s.tokenRepository.Save(chatID, authResp.AccessToken, rep.AccessTokens)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
